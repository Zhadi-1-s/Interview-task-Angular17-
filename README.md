...
Детали заказа с редактированием позиций и пересчётом суммы

Optimistic update с откатом при ошибке

Тост-уведомления, спиннеры загрузки, пустые состояния

Глобальная обработка 4xx/5xx через Interceptor

Query-параметры для фильтров и пагинации

Фейковая авторизация (любая пара email/password работает)

Кэширование списка заказов через BehaviorSubject/Signals
